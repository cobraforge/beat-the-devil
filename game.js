...
function drawFinger(x0, y0, a1, len, curl, r0, r1, bendSign, fire){
  ...
  // the creases, only where the flesh folds — across the underside between
  // the knuckles, bowed toward the tip, fading out before the upper edge
  [0.33, 0.59].forEach(function(u){
    ...
    var cg2 = ctx.createLinearGradient(left[kk].x, left[kk].y, right[kk].x, right[kk].y);
    ...
    ctx.beginPath();
    ctx.moveTo(lerp(left[kk].x, c.x, 0.45), lerp(left[kk].y, c.y, 0.45));
    ctx.quadraticCurveTo(mid.x, mid.y, right[kk].x, right[kk].y);
    ctx.stroke();
  });
...
function drawArmBody(curl, spread, fire){
  ...
  ctx.beginPath();
  ctx.moveTo(-40, -wr);
  ctx.bezierCurveTo(-16, -52, 12, -54, 30, -40);                // the thumb's mound, on top
  ctx.bezierCurveTo(46, -26, 52, -12, 50, -2);                  // out to the knuckle of the first finger
  ctx.bezierCurveTo(56, 12, 54, 34, 44, 46);                    // the leading edge the fingers leave from
  ctx.bezierCurveTo(24, 58, -12, 56, -40, wr + 4);
  ctx.bezierCurveTo(-22, wr - 10, -22, -wr + 8, -40, -wr);
  ctx.closePath(); ctx.fill();
  ctx.save(); ctx.clip();
  hideOver(0.6);
  ...
  for (var i=0;i<4;i++){
    var rx = 34, ry = -8 + i * 16, ext = 1 - curl;
    ctx.strokeStyle = 'rgba(0,0,0,' + (0.22 + 0.2 * ext) + ')'; ctx.lineWidth = 3.4;
    ctx.beginPath(); ctx.moveTo(-28, -14 + i * 9); ctx.quadraticCurveTo(4, ry - 4, rx, ry); ctx.stroke();
    ctx.strokeStyle = 'rgba(255,150,130,' + (0.16 + 0.2 * ext) + ')'; ctx.lineWidth = 1.4;
    ctx.beginPath(); ctx.moveTo(-28, -16 + i * 9); ctx.quadraticCurveTo(4, ry - 6, rx, ry - 2); ctx.stroke();
  }
  // the knuckle ridge where the fingers root: four swellings along the lower edge
  for (i=0;i<4;i++){
    var kx = 34 + (i === 0 || i === 3 ? -4 : 2), ky = -8 + i * 16;
    var kg = ctx.createRadialGradient(kx - 3, ky, 1, kx, ky, 11);
  ...
  var roots = [[40, -6, -0.30], [46, 10, -0.10], [46, 26, 0.10], [40, 40, 0.30]];
  var lens = [62, 76, 74, 60];
  for (i=0;i<4;i++){
    var rt = roots[i];
    drawFinger(rt[0], rt[1], rt[2] * (0.55 + 0.9 * spread), lens[i], curl, 9.5 - Math.abs(i - 1.5) * 0.7, 5.2, 1, fire);
  }
  // --- the thumb: off the upper edge, reaching in above the fingers and
  //     curling down over whatever the hand closes on
  drawFinger(24, -34, -0.34 + 0.22 * spread, 66, curl * 0.95 + 0.16, 11, 6.2, 1, fire);
  ctx.restore();
}

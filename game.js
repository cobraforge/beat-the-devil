...
function drawArmBody(curl, spread, fire){
  ...
  var hollow = ctx.createRadialGradient(6, 2, 2, 6, 2, 46);
  ...
  [[34, -6, 13], [36, 10, 13], [33, 26, 12], [26, 38, 10], [-2, -30, 18], [-26, 22, 16]].forEach(function(m){
  ...
  pg.addColorStop(0, 'rgba(0,0,0,.42)'); pg.addColorStop(0.5, 'rgba(0,0,0,0)'); pg.addColorStop(1, 'rgba(255,130,50,' + (0.16 + 0.3 * fire) + ')');
  ...
  // They splay like a bird's foot: the top one reaches slightly up, the
  // bottom one down, and each only hooks at the tip. A full curl here would
  // fold them into a fist and they would braid together.
  var fingers = [
    [28, 44, 0.62, 52, 7.4, 0.55],     // the outermost, rooted furthest back
    [39, 25, 0.28, 66, 8.2, 0.25],
    [45,  4, -0.08, 78, 8.8, 0.0],     // the longest, nearest the thumb
    [40, -17, -0.44, 66, 8.2, 0.3]
  ];
  var grip = 0.3 + 0.45 * curl;                                 // never a fist
  fingers.forEach(function(f){
    ...
    if (f[5] > 0){                                              // and a little darker still
      ...
      ctx.fillStyle = '#000';
      ctx.beginPath(); ctx.ellipse(f[0] + f[3] * 0.4, f[1] + f[3] * 0.25, f[3] * 0.6, f[3] * 0.45, 0, 0, 6.2832); ctx.fill();
    }
    ctx.restore();
  });
  // --- the thumb: short, thick and opposed, coming off the mount on the upper
  //     edge and closing down across the fingers rather than running beside them
  drawFinger(14, -32, 0.5 + 0.18 * spread, 44, 0.35 + 0.4 * curl, 11.5, 6.6, 1, fire);
  ctx.restore();
}

...
function resize(){
  ...
  scale = Math.min(aw / (LW + BEZEL_X), ah / (LH + BEZEL_Y));
...
var faceFire = FIRE.make(128, 96);
var colFire  = FIRE.make(96, 144);
...
var pointerDown = false, lastPX = 0, lastPY = 0, downT = 0, downMode = '', moved = 0;
...
stage.addEventListener('pointerdown', function(e){
  ...
  pointerDown = true;
  var p = toLogical(e); lastPX = p.x; lastPY = p.y;
  downT = performance.now(); downMode = G.mode; moved = 0;
  if (G.mode === 'play'){ if (G.paused) togglePause(); }
  else tryStart();
});
...
function endPointer(){
  if (pointerDown && downMode === 'play' && G.mode === 'play' && !G.paused &&
      moved < 10 && performance.now() - downT < 300){
    fireQueued = true;   // a tap (not a drag) fires
  }
  pointerDown = false;
}
...
function burst(x, y, color, n, spd, grav){
  for (var i=0;i<(n||8);i++){
    ...
    G.parts.push({ x:x, y:y, vx: Math.cos(a)*s, vy: Math.sin(a)*s, life: rnd(0.3,0.75), t:0, c: color, r: rnd(1.3,3.2), g: grav == null ? 260 : grav });
  }
}
  ...
  for (var i=G.flames.length-1;i>=0;i--){
    ...
    if (fl.type === 'walker'){
      ...
      if (fl.h > 30 && Math.random() < 0.4)
        G.parts.push({ x: fl.x + rnd(-fl.w*0.3, fl.w*0.3), y: LH - fl.h + rnd(-6, 24), vx: rnd(-12, 12), vy: rnd(-70, -30),
      ...
      if (fl.state === 'fly'){
        ...
        if (Math.random() < 0.6){
          ...
          G.parts.push({ x: fl.x - fl.vx / bv * rnd(8, 26) + rnd(-4, 4), y: fl.y - fl.vy / bv * rnd(8, 26) + rnd(-4, 4),
...
function hurt(kind, hx, hy, vx, vy){
  ...
  for (var li=0; li<16; li++)
    G.parts.push({ x: p.x, y: p.y, vx: rnd(-90, 90), vy: rnd(-120, 20), life: rnd(0.5, 1.1), t: 0,
...
function drawPlayer(){
  ...
  if (speed > 150 && Math.random() < 0.8)
    G.parts.push({ x: p.x - p.vx / speed * 8 + rnd(-3, 3), y: y - p.vy / speed * 8 + rnd(-3, 3), vx: -p.vx * 0.05, vy: -p.vy * 0.05,
                   life: rnd(0.18, 0.32), t: 0, c: col('heart', 0.5), r: rnd(1.5, 3), g: 0 });
  if (burn && speed > 40 && Math.random() < 0.5)
    G.parts.push({ x: p.x + rnd(-6, 6), y: y + rnd(-6, 6), vx: -p.vx * 0.06 + rnd(-8, 8), vy: -p.vy * 0.06 - 30, life: rnd(0.5, 1.0), t: 0,
  ...
  if (G.embed){
    ...
    ctx.shadowColor = COLORS.ember; ctx.shadowBlur = 8;
    ctx.strokeStyle = '#ff6a24'; ctx.lineWidth = 3;
    forkShape();
    ctx.restore();
    if (Math.random() < 0.7)
      G.parts.push({ x: p.x + em.dir.x * 6 + rnd(-3, 3), y: y + em.dir.y * 6 + rnd(-3, 3), vx: em.dir.x * 20 + rnd(-10, 10), vy: -18 + rnd(-10, 10),
  ...
  if (burn){
    ...
    var fh = 26 * bk;
    ctx.save();
    ctx.shadowColor = '#ff6a10'; ctx.shadowBlur = 14;
    ...
    ctx.restore();
    if (Math.random() < 0.4)
      G.parts.push({ x: bx + rnd(-4, 4), y: by - fh * 0.6, vx: rnd(-8, 8), vy: rnd(-40, -20), life: rnd(0.3, 0.7), t: 0, c: '#ffb060', r: rnd(0.8, 1.6), g: -20, turb: 30 });
  }
  ctx.restore();
}
...
function drawFork(f){
  ...
  ctx.shadowBlur = f.state === 'lock' ? 16 : 8;
...
function flameColumn(x, h, w, seed, flare){
  ...
  ctx.closePath();
  ctx.shadowColor = '#ff5a10'; ctx.shadowBlur = 26 + 20 * (flare || 0);
  ...
  ctx.shadowBlur = 0;
  ctx.clip();
  var sx = Math.floor(seed * 60) % 60;
  ctx.drawImage(colFire.canvas, sx, 0, 36, colFire.h, x - w*0.7, top - 10, w*1.4, h + 14);
  if (flare){
    ctx.fillStyle = 'rgba(255,240,200,' + (0.5 * flare) + ')';
    ctx.fillRect(x - w, top - 20, w * 2, h + 30);
  }
  ctx.restore();
  // heat shimmer: everything seen through and just above the flame is displaced sideways
  var S = cvs.width / LW, sh = 7;
  ctx.save();
  ctx.setTransform(1, 0, 0, 1, 0, 0);
  for (var yy = Math.max(2, top - 70); yy < LH - 6; yy += sh){
    var above = yy < top ? 1 - (top - yy) / 70 : 1;
    var amp = yy < top ? 6 * above : 2.6;
    var off = (FIRE.at(yy * 0.22 + seed * 200, t * 85) - 0.5) * 2 * amp;
    var sy = Math.floor(yy * S), sxx = Math.max(0, Math.floor((x - w * 0.9) * S));
    var sw = Math.floor(w * 1.8 * S), shh = Math.ceil(sh * S);
    if (sy + shh > cvs.height) break;
    ctx.drawImage(cvs, sxx, sy, sw, shh, sxx + off * S, sy, sw, shh);
  }
  ctx.restore();
}
function drawFlame(fl){
  if (fl.type === 'walker'){
    ...
    ctx.closePath();
    ctx.shadowColor = COLORS.ember; ctx.shadowBlur = 30;
...
function drawFireball(f){
  ...
  ctx.shadowColor = '#ff6a10'; ctx.shadowBlur = 18;
...
function drawScorchRing(f){
  ...
  ctx.save();
  ctx.shadowColor = '#ff6a10'; ctx.shadowBlur = 6 + 18 * k;
  ...
  ctx.shadowColor = COLORS.ember; ctx.shadowBlur = 6 + 12 * k;
...
function drawDetonation(f){
  ...
  ctx.fillStyle = g; ctx.shadowColor = COLORS.ember; ctx.shadowBlur = 16;
...
function drawJet(j){
  ...
  ctx.closePath();
  ctx.shadowColor = '#ff5a10'; ctx.shadowBlur = 24;
...
function drawDevil(d){
  ...
  [-1, 1].forEach(function(dir){
    ...
    ctx.shadowColor = '#ff6a10'; ctx.shadowBlur = 10;
  ...
  ctx.save();
  skullPath(skull);
  ctx.shadowColor = '#ff4a00'; ctx.shadowBlur = 24;
  ...
  ctx.save();
  skullPath(mpts);
  ctx.shadowColor = '#ff8a20'; ctx.shadowBlur = 18 + 30 * glow;
...
function drawGem(x, y, s, o){
  ...
  ctx.shadowColor = rgba(core[0], core[1], core[2], 1);
  ctx.shadowBlur = (10 + 14 * pulse) * glow;
  ...
  scars.forEach(function(sc){
    if (sc.kind === 'char'){
      ...
      if (o.leak && s > 8 && Math.random() < 0.35){
        ...
        G.parts.push({ x: x + pt2[0]*s, y: y + pt2[1]*s, vx: rnd(-14, 14), vy: rnd(-30, -8), life: rnd(0.4, 0.9), t: 0,
...
function drawQuiver(){
  ...
  for (var i=0;i<AMMO;i++){
    ...
    if (i >= G.ammo){
      ...
      ctx.shadowColor = COLORS.heart; ctx.shadowBlur = 8;
      ctx.fillStyle = '#eefaff';
      boltShape(x, y, 0.75);
      ctx.fill();
    }
...
function drawHerald(){
  ...
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.shadowColor = COLORS.heart; ctx.shadowBlur = 24;
...
function drawPaused(){
  ...
  ctx.shadowColor = COLORS.sulfur; ctx.shadowBlur = 12;
...
function draw(){
  ...
  if (needFire){ colFire.render(G.t, 'column'); }
  if (G.devil && G.devil.state !== 'wait' && !G.devil.dead) faceFire.render(G.t, 'face');
  ...
  var hg = ctx.createLinearGradient(0, LH, 0, LH - 260);
  hg.addColorStop(0, rgba(M[0], M[1], M[2], mist));
  hg.addColorStop(1, rgba(M[0], M[1], M[2], 0));
  ctx.fillStyle = hg;
  ctx.fillRect(-20, LH - 260, LW + 40, 280);
  ...
  if (G.lightning > 0 && G.phase === 'devil') drawLightning();
  if (G.watch) drawWatch();

  var taken = G.taken && G.mode === 'ending';
  if (G.devil) drawDevil(G.devil);
  if (G.arms && !taken) drawArms();
